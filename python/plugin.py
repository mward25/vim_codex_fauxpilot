...
MAX_SUPPORTED_INPUT_LENGTH = 1000
...
def complete_input_max_length(input_prompt, max_input_length=MAX_SUPPORTED_INPUT_LENGTH, stop=None, max_tokens=1000):
    input_prompt = input_prompt[-max_input_length:]
    #response = openai.Completion.create(engine='codegen', prompt=input_prompt, best_of=10, temperature=0.5, max_tokens=max_tokens, stream=USE_STREAM_FEATURE, stop=stop)
    response = openai.Completion.create(engine='codegen', prompt=input_prompt, temperature = 0.1, stop=stop)#, temperature=0.5, max_tokens=max_tokens, stop=stop)
    return response
